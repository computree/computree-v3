...
{
    QString err;
    CT_LASHeader *header = readHeader(filepath, err);
...
CT_LASHeader* CT_Reader_LAS::readHeader(const QString &filepath, QString &error) const
{
...
bool CT_Reader_LAS::protectedReadFile()
{
    ...
    if(header->m_pointDataRecordFormat != ((CT_LASHeader*)m_header)->m_pointDataRecordFormat) {
        PS_LOG->addErrorMessage(LogInterface::reader, tr("Le format (%1) du fichier %2 ne correspond pas au format (%3) du fichier lu lors de la configuration").arg(header->m_pointDataRecordFormat)
                                .arg(filepath())
                                .arg(((CT_LASHeader*)m_header)->m_pointDataRecordFormat));
        return false;
    }
