#include "pb_stepapplypointfiltersingrid.h"
...
PB_StepApplyPointFiltersInGrid::PB_StepApplyPointFiltersInGrid(CT_StepInitializeData &dataInit) : CT_AbstractStep(dataInit)
{
}

PB_StepApplyPointFiltersInGrid::~PB_StepApplyPointFiltersInGrid()
{
...
void PB_StepApplyPointFiltersInGrid::init()
{
...
QString PB_StepApplyPointFiltersInGrid::getStepDescription() const
{
    return tr("Filtres de points (dans une grille)");
}

// Step detailled description
QString PB_StepApplyPointFiltersInGrid::getStepDetailledDescription() const
{
    QString ret;
...
// Step URL
QString PB_StepApplyPointFiltersInGrid::getStepURL() const
{
...
// Step copy method
CT_VirtualAbstractStep* PB_StepApplyPointFiltersInGrid::createNewInstance(CT_StepInitializeData &dataInit)
{
    return new PB_StepApplyPointFiltersInGrid(dataInit);
}

//////////////////// PROTECTED METHODS //////////////////

// Creation and affiliation of IN models
void PB_StepApplyPointFiltersInGrid::createInResultModelListProtected()
{
    CT_InResultModelGroupToCopy *resIn_res = createNewInResultModelForCopy(DEFin_res, tr("Points"));
    resIn_res->setZeroOrMoreRootGroup();
    resIn_res->addGroupModel("", DEFin_grpRoot, CT_AbstractItemGroup::staticGetType(), tr("Groupe racine"));
    resIn_res->addItemModel(DEFin_grpRoot, DEFin_lasAtt, CT_StdLASPointsAttributesContainer::staticGetType(), tr("Attributs LAS"), "", CT_InAbstractItemModel::C_ChooseOneIfMultiple, CT_InAbstractItemModel::F_IsOptional);
    resIn_res->addGroupModel(DEFin_grpRoot, DEFin_grp, CT_AbstractItemGroup::staticGetType(), tr("Groupe"));
...
bool PB_StepApplyPointFiltersInGrid::postConfigure()
{
...
SettingsNodeGroup* PB_StepApplyPointFiltersInGrid::getAllSettings() const
{
    SettingsNodeGroup *root = CT_VirtualAbstractStep::getAllSettings();
    SettingsNodeGroup *group = new SettingsNodeGroup("PB_StepApplyPointFiltersInGrid");
...
bool PB_StepApplyPointFiltersInGrid::setAllSettings(const SettingsNodeGroup *settings)
{
    qDeleteAll(_selectedFilters);
    _selectedFilters.clear();
    ...
    if(ok)
    {
        SettingsNodeGroup* group = settings->firstGroupByTagName("PB_StepApplyPointFiltersInGrid");
...
void PB_StepApplyPointFiltersInGrid::createOutResultModelListProtected()
{       
...
void PB_StepApplyPointFiltersInGrid::compute()
{
    ...
    while (itCpy_grpRoot.hasNext() && !isStopped())
    {
        ...
        const CT_StdLASPointsAttributesContainer* lasAtt = (CT_StdLASPointsAttributesContainer*)grpRoot->firstItemByINModelName(this, DEFin_lasAtt);

        CT_GroupIterator itCpy_grp(grpRoot, this, DEFin_grp);
